...
- `<카테고리>.draft.md`: 검토 중인 후보 규칙 모음.
- `<카테고리>.md`: 큐레이션 결과물.

두 파일 모두 위에 정의한 4개 본문 블록 구조(설명 → 근거 → 설정 → 예시)와 예시 라벨 규칙을 동일하게 준수한다. 옵션이 없는 규칙에 `없음`을 명시하는 정책도 양쪽에 똑같이 적용된다. **유일한 차이는 정식 파일만 `src/rules/<카테고리>.ts`에 반영된다는 점이다.**

규칙을 draft에서 정식 파일로 승격할 때:

1. draft에 작성해 둔 항목을 정식 `.md`로 옮기고 `src/rules/<카테고리>.ts`를 같은 turn에 갱신한다.
2. **draft에서 해당 항목을 즉시 제거한다.** 두 곳에 같은 규칙이 남으면 어느 쪽이 정책인지 모호해진다.
