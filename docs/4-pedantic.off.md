...
`if (!x) A else B`나 `!x ? a : b`처럼 부정 조건을 우선 분기에 두면 흐름이 거꾸로 읽힌다는 규칙. oxlint에서 두 규칙은 `if`와 삼항을 모두 잡는 동치 규칙이다.
...
**취향.** TODO와 FIXME는 진행 중 작업을 표시하는 의도된 마커이고, lint 에러로 잡으면 매번 disable로 우회하거나 메모를 강제로 지우게 되어 인라인 메모와 이슈 트래커의 역할이 흐려진다.
...
**취향.** TanStack Router는 인증/리디렉션을 `throw redirect({...})`로 처리하도록 권장하는데, `redirect()`는 Error 서브클래스가 아닌 라우터 컨트롤 플로우 객체라 규칙이 false positive로 모두 잡아 카테고리째 끈다.
...
변경 가능한 객체 파라미터에 `readonly` 속성 사용을 강제해 함수가 입력을 변경하지 않는다는 계약을 타입으로 표현한다.

**취향.** 모든 객체 파라미터마다 `readonly` 키워드가 늘어나 시그니처 가독성이 크게 떨어지고, 외부 라이브러리 타입이 readonly가 아닐 때 가짜 위반도 폭증한다.
...
`void` 반환 타입의 콜백에서 값을 반환하지 못하게 하는 규칙. 호출자가 반환값을 무시한다는 계약을 보호한다.
