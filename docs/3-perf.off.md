...
`Array.prototype.map` 콜백 안에서 객체/배열 spread(`{ ...item }`, `[...arr]`)로 새 값을 만들면 항목마다 얕은 복사가 발생한다고 보고, `Object.assign(item, ...)`로 원본을 제자리에서 변경하는 패턴을 권한다.
...
- `ignoreArgs` (bool, default: `true`): 배열을 함수 인자로 전달하는 경우 보고 제외
