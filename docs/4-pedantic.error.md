...
```ts
// helpers.ts, constants.ts, foo.tsx 로 책임 분리
```
...
- `checksVoidReturn.inheritedMethods` (bool, default: `true`): void 반환 상속 메서드 오버라이드
...
async 함수를 wrapper로 감싸 fire-and-forget을 명시한다.
