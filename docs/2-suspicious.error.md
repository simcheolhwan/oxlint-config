...
`catch` 블록에서 새 `Error`를 던질 때 원본 error를 잃으면 원인 추적이 어려워진다. `throw new Error("...", { cause: caughtError })`처럼 `cause`로 원본을 보존한다.

**베스트 프랙티스.** `cause` 한 줄로 디버깅 시간이 크게 줄고 추가 비용이 거의 없으며, `requireCatchParameter: true`로 optional catch binding(`catch {}`)까지 차단해 무시 의도와 빠뜨린 처리를 코드로 구분한다.
...
같은 모듈에 동일 이름의 named export가 있는데 default import 식별자를 그 이름으로 받는 패턴을 잡는다. default import 이름을 다른 식별자로 바꾸거나 named import로 명시해 분리한다.
...
바인딩 없이 사이드 이펙트로만 import하는 패턴(`import "x"`)을 금지한다. 사이드 이펙트의 발생 지점을 명시적인 호출로 드러내도록 강제한다.

**베스트 프랙티스.** CSS는 빌드 도구가 자산으로 처리하는 표준 패턴이라 `allow` 옵션으로 화이트리스트하고, 차트 라이브러리 등 사이드 이펙트 진입점을 권장하는 경우도 라이브러리 특성에 따라 추가할 수 있다.
...
이미 `boolean` 타입인 식별자를 `=== true`/`!== false`로 비교하는 잡음을 잡는다. 변수를 그대로 사용하고 부정이 필요하면 `!`만 붙인다.

**베스트 프랙티스.** 이름(`isOpen`, `hasError` 등)이 이미 boolean임을 시사하므로 명시 비교는 가독성을 해치고, 자동 수정으로 일괄 정리되어 도입 비용이 거의 없다.
...
제네릭 정의의 default와 동일한 타입 인자를 호출부에 명시한 패턴을 잡는다. 정의부 default가 호출 의도와 일치하면 호출부에서도 default를 따라야 일관성이 유지되고, default 변경 시 한 곳만 수정하면 된다.

**베스트 프랙티스.** `no-unnecessary-type-parameters`가 정의부의 가짜 제네릭을 잡는 반면 이 규칙은 호출부에서 default와 같은 타입 인자를 잡아 같은 의도의 짝을 이루며, auto-fix가 제공되어 일괄 정리 비용도 거의 없다.
...
함수에 선언한 타입 파라미터가 입력, 반환 타입을 실제로 연결하지 못하고 본문에서 단순 cast로 소비되면 잡는다. 제네릭을 제거하고 반환 타입을 `unknown`으로 두거나, 입출력을 실제로 묶는 형태로 다시 설계한다.

**베스트 프랙티스.** `parseYAML<T>(...)` 류의 false generic은 호출 측에 잘못된 타입 안전감을 주지만 런타임 검증은 없어, 잘못된 시그니처가 코드베이스에 굳어지기 전에 정적으로 차단한다.
...
**베스트 프랙티스.** 클로저 의존이 없는 헬퍼는 모듈 함수가 더 자연스러워 `.ts`에서는 켜되, `.tsx`는 컴포넌트 전용 이벤트 핸들러와 렌더 헬퍼가 클로저 의존이 없어도 컴포넌트에 응집시키는 편이 자연스럽고 곧 props/state를 참조하게 되어 모듈로 올렸다 되돌리는 왕복이 잦아, false positive가 과하므로 `**/*.tsx` override로 끈다.
...
`Array#sort()`는 원본 배열을 그 자리에서 변경해 호출자가 예측하기 어렵다. 새 배열을 반환하는 `Array#toSorted()`(ES2023)를 사용한다.
