...
화살표 함수 본문 표기를 표현식(`() => x`) 또는 블록(`() => { return x }`) 중 하나로 통일하도록 강제한다.

**취향.** 나중에 본문에 훅이나 부수 효과를 추가하면 다시 블록으로 바꿔야 하는 경우가 많고, 단순 반환은 표현식, 다단계 동작은 블록이 자연스러우므로 두 형태를 자유롭게 혼용한다.
...
함수 정의 표기를 `function` 선언 또는 화살표 함수 표현식 중 하나로 통일하도록 강제한다.
...
- `checkGeneric` (bool, default: `true`): TypeScript 제네릭 매개변수 이름 검사 여부
...
JSX 이벤트 핸들러 prop 이름과 함수 이름이 접두사 관례(`on*` prop, `handle*` 함수)를 따르도록 강제한다.

**베스트 프랙티스.** TanStack Query의 `mutate`, 스토어의 `setOpen`, prop으로 받은 `onChange` 같은 외부 콜백을 그대로 JSX prop에 전달하는 패턴이 흔해, 일률적으로 강제하면 단순 위임 래퍼 함수(`const handleChange = (v) => onChange(v)`)가 필요해져 노이즈가 커진다.

**Configuration**

- `eventHandlerPrefixes` (string, default: `"handle"`): 핸들러 함수 이름 접두사
- `eventHandlerPropPrefixes` (string, default: `"on"`): 핸들러 prop 이름 접두사
- `eventHandlerRegex` (regex string, default: 없음): 핸들러 함수 이름 정규식 (접두사 대신 사용)
- `eventHandlerPropRegex` (regex string, default: 없음): 핸들러 prop 이름 정규식 (접두사 대신 사용)
...
**취향.** named 인덱스 시그니처(`{ [userId: string]: number }`)가 키의 도메인 의미를 드러내 원래 `"index-signature"` 강제를 선호했으나, 자동 수정이 의미 있는 이름을 추론하지 못하고 모든 키를 `{ [key: string]: V }`로 변환해 그 이점이 사라지므로, 의미 있는 키 이름은 사람이 직접 붙이도록 규칙을 끈다.
...
**베스트 프랙티스.** `main.tsx`, `routes.tsx`, `[id].tsx` 같은 프레임워크와 라우팅 관례의 예외가 얼마든지 발생할 수 있어 일률 강제가 어렵고, 어떤 대소문자 옵션을 켜도 다른 파일 이름을 오탐으로 보고한다.
...
프레임워크와 라우팅 관례의 예외(`main`, `routes` 등).
...
숫자 리터럴의 `_` 자릿수 구분자 스타일(예: `1_000_000`)을 강제한다.
...
**베스트 프랙티스.** 브라우저 코드에서 `window.prompt`/`window.confirm`이 관용적이고 환경 의도를 명확히 표현하는 반면 `globalThis.prompt`는 비관용적이고, 범용 모듈에서는 명시적 `typeof window !== "undefined"` 가드가 더 적절하다.
...
**취향.** 함수 참조 방식은 이름 변경 시 자동 동기화 이점이 있지만 테스트 출력 가독성과 일반적인 관례에 어긋나고, 같은 함수에 대해 여러 시나리오를 `describe`로 묶을 때 문자열이 더 자연스럽다.
