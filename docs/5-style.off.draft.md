...
가능한 경우 `obj["prop"]` 대신 `obj.prop` dot notation을 사용하도록 강제한다.

**베스트 프랙티스.** 표기 차이가 가독성에 큰 영향을 주지 않고, 환경 변수나 설정 키처럼 문자열로 다루는 게 자연스러운 접근에서는 bracket 표기가 의도를 더 잘 드러낸다.
...
추론 가능한 원시 타입의 명시적 표기를 금지하고 타입 추론에 맡기도록 요구한다.
...
TypeScript 클래스의 parameter property(`constructor(private x: number)`) 사용을 금지한다.

**베스트 프랙티스.** parameter property는 TypeScript 표준 단축 표기로 보일러플레이트를 줄여주고, React 코드베이스에서 클래스 사용 자체가 드물어 강제 금지의 실익이 작다.
