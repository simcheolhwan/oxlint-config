...
객체와 배열의 rest 및 spread 문법(`{ ...obj }`, `[...arr]`)을 금지한다.

**베스트 프랙티스.** spread는 불변 패턴의 핵심 표현이라 금지하면 코드가 더 장황해진다.

**Configuration**

- `objectRestMessage` (string, default: `""`): object rest 감지 시 표시할 사용자 지정 메시지
- `objectSpreadMessage` (string, default: `""`): object spread 감지 시 표시할 사용자 지정 메시지
...
모든 `test`/`it` 호출에 명시적 타임아웃 인자(또는 `vi.setConfig({ testTimeout })` 전역 설정)를 강제한다.

**취향.** 테스트별 타임아웃이 의미 있는 경우는 일부 느린 통합 테스트에 한정되고 전역 타임아웃 설정으로 충분히 다뤄지며, 모든 테스트에 타임아웃 인자를 추가하면 시그니처가 노이즈로 가득 차 가독성이 떨어진다.
