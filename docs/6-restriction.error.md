...
모든 `switch` 문에 `default` 절을 강제해 fall-through로 인한 무음 버그를 차단한다. 의도적으로 default를 비울 때는 `// no default` 코멘트로 명시한다.

**베스트 프랙티스.** `typescript/switch-exhaustiveness-check`(pedantic)가 컴파일 타임에 알려진 union/enum 분기를 강제하는 반면 이 규칙은 분기 종류와 무관하게 명시적 `default`를 요구해 외부 입력이나 임의 문자열 같은 런타임 값에도 안전망을 만든다.
...
**베스트 프랙티스.** SPA 환경에서 네이티브 다이얼로그는 디자인 시스템과 일관성이 깨지고, 운영 알림은 토스트나 모달, 확인은 커스텀 컨펌 다이얼로그로, 디버깅 흔적은 `console.warn`/`console.error`(`no-console`의 `allow`로 허용)로 처리한다.
...
**베스트 프랙티스.** `console.log`는 디버깅 임시 코드의 대표 흔적이라 차단하고, `warn`/`error`/`info`는 의도된 사용자 메시지이거나 비정상 흐름 알림이라 `allow`로 풀어 임시 코드만 정조준한다.
...
빈 본문을 가진 함수, 메서드를 금지한다. 의도된 noop인지 미완성 코드인지 구분되지 않아 PR 리뷰에서 무음 버그로 흘러가기 쉽다.

**베스트 프랙티스.** `allow: ["arrowFunctions"]`로 빈 화살표 함수만 풀어 props 기본값이나 Context 초기값의 noop 관용구를 허용하고, 함수 선언/메서드/생성자의 빈 본문은 미완성 신호가 강해 그대로 잡는다.
...
식별자를 정의 전에 사용하면 호이스팅에 의존한 사용 흐름이 만들어진다. 변수, 클래스는 TDZ 또는 런타임 에러로 이어지므로 잡고, 함수는 호이스팅이 안전해 풀어준다.

**베스트 프랙티스.** `functions: false`로 함수 호이스팅만 허용해 "메인 컴포넌트는 파일 상단, 보조 함수는 그 아래" 같은 코드 정렬 패턴(특히 TanStack Router의 `createFileRoute(...)({ component: Page })`)을 유지하고, 변수, 클래스 호이스팅은 위험 신호라 그대로 잡는다.
...
`void` 연산자는 표현식 결과를 undefined로 만드는 역사적 문법이라 일반 표현식 안에 섞이면 의도가 모호해진다. statement 형태(`void promise`)만 풀어두면 fire-and-forget Promise 관용구를 유지하면서도 식 안의 `void` 사용을 차단할 수 있다.
...
`../`로 부모 디렉터리를 거슬러 올라가는 import를 금지한다. 같은 디렉터리, 하위 디렉터리 import(`./`)와 외부 패키지, alias import는 그대로 허용한다.
...
**베스트 프랙티스.** 모듈/스크립트 모호성을 없애 도구 추론과 사용자 기대를 일치시키고, ambient declaration 파일(`*.d.ts`)은 `declare module`, `interface` 선언만 두고 export를 비우는 정상 패턴이라 override로 끈다.
...
`<button>`의 `type` 속성이 누락되면 HTML 기본값이 `"submit"`이라 폼 내부에 놓인 버튼이 의도하지 않은 폼 제출을 일으킨다. `type="button"`을 명시해 클릭과 Enter 키가 폼 제출로 새지 않도록 한다.
...
함수와 메서드의 반환 타입(`explicit-function-return-type`) 또는 export 함수의 매개변수와 반환 타입(`explicit-module-boundary-types`)을 명시하도록 요구한다. 모듈 경계 계약을 코드에 박아 두면 의도된 반환 타입과 실제 추론 결과가 어긋날 때 호출부가 아닌 정의부에서 즉시 노출된다.

**베스트 프랙티스.** `.ts` 파일에서 두 규칙을 켜되 `explicit-function-return-type`에는 `allowExpressions: true`를 함께 켜 `users.map((user) => user.id)` 같은 인라인 콜백을 풀어 두고, `.tsx`는 React 컴포넌트의 `JSX.Element` 추론과 props 타입의 명시성이 이미 충분해 override로 끈다.
...
`delete obj[expr]`처럼 computed key에 대한 `delete` 사용을 금지하고 정적 접근(`delete obj.key`)만 허용한다.
...
`!` non-null assertion은 타입 시스템을 강제 우회한다. 타입 가드로 좁히거나 nullable을 처리한다.
...
Promise를 반환하는 함수가 `async`로 표시되지 않으면 호출 계약이 덜 명확해 `async function`으로 선언한다.

**베스트 프랙티스.** React 19에서 `ReactNode = ... | Promise<ReactNode>`로 확장되어 화살표 핸들러가 false positive로 잡히므로 `checkArrowFunctions: false`로 화살표 함수를 제외하고 함수 선언의 Promise 반환에만 `async`를 강제한다.
...
**베스트 프랙티스.** 이름 없는 disable은 의도한 한 규칙뿐 아니라 같은 라인의 다른 진단까지 무음으로 만들고 PR 리뷰에서도 추적되지 않아 lint 신뢰가 무너지므로 항상 규칙 이름을 명시하도록 강제한다.
...
`Array#forEach` 대신 `for...of` 사용을 강제한다. 콜백이 새 함수 경계를 만들어 TypeScript의 타입 좁힘이 끊기고, `break`/`continue`/`return`으로 조기 종료가 불가능하며, 콜백 호출 오버헤드만큼 느리다.

**베스트 프랙티스.** `for...of`는 외부 스코프의 type narrowing을 유지하고 async 컨텍스트에서 `await`을 그대로 쓸 수 있으며 조기 종료까지 지원해, 명시적 반복문으로 통일하면 의도와 성능을 둘 다 챙긴다.
...
`Array#reduce`/`reduceRight`는 누산 로직을 한 줄 표현식에 압축해 가독성을 떨어뜨리고 spread 누적(`(acc, x) => [...acc, x]`)처럼 O(n²) 성능 함정으로 흐르기 쉽다. `map`/`filter`/`for...of`로 풀어 쓰면 의도와 비용이 모두 분명해진다.
...
전역 `parseInt`/`parseFloat`/`isNaN`/`isFinite`/`NaN` 대신 `Number.parseInt`/`Number.isNaN` 같은 `Number` 정적 멤버를 강제한다. ES2015에서 `Number`로 이동한 이후 글로벌 버전과 동작이 미묘하게 달라 출처를 명시해야 안전하다.

**베스트 프랙티스.** 글로벌 `isNaN("foo")`는 인자를 숫자로 강제 변환한 뒤 `NaN` 여부를 따져 `true`를 반환하지만 `Number.isNaN("foo")`는 강제 변환 없이 `false`를 반환하므로 모듈 경계가 분명한 `Number.*`로 의도가 다르게 흘러갈 여지를 차단한다.
