...
- `Object.assign({}, src)` 대신 `{ ...src }` spread 사용을 강제하는 룰.
...
- 익명 값의 기본 export를 금지한다.
- 이름 있는 변수에 할당한 뒤 export한다.

### 근거

- **베스트 프랙티스.** 디버깅, 스택 트레이스에 이름이 보여 도움이 되지만, default export를 안 쓰는 정책과 효과가 겹친다.
...
- 자식이 없는 React 컴포넌트는 self-closing(`<Foo />`)으로 닫도록 강제하는 룰.
...
- `for (let i = 0; i < arr.length; i++)` 같은 인덱스 순회를 `for-of` 루프로 대체하도록 요구한다.
- 인덱스 변수와 수동 배열 접근이 사라져 가독성이 좋아지고 off-by-one 같은 실수도 줄어든다.
...
- 변경되지 않는 클래스 멤버에 `readonly` 표시를 강제하는 룰.
...
- `(await x).y` 같이 await 결과에서 직접 멤버 접근을 금지하는 룰.

### 근거

- **베스트 프랙티스.** `await`의 우선순위가 헷갈려 버그를 유발한다.
- 변수에 분리하면 의도가 분명해진다.
