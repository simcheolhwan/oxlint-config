...
**취향.** 가드로 짧게 빠지고 본 경로에서만 결과를 반환하는 early return 흐름이 이 코드베이스의 기본 스타일이고, 반환 타입은 TypeScript 시그니처(`T | undefined` 등)로 충분히 표현되므로 규칙 차원의 강제는 과하다.
...
`any`/`unknown` 등 더 넓은 타입에서 좁은 타입으로의 `as` 단언을 안전하지 않다고 보고 차단한다. 좁히려면 type guard를 거쳐야 한다.
