...
- `countThis` (`"always" | "never" | "except-void"`, default: 없음): `this` 매개변수 포함 여부
- `countVoidThis` (bool, default: `false`): [deprecated] `this: void` 매개변수 포함 여부
...
**취향.** 기본값 `max: 10`은 일반 헬퍼와 React 컴포넌트에서 자주 초과돼 노이즈가 크므로 `.ts`/`.tsx` 구분 없이 `max: 20`으로 완화하고, 테스트 파일은 시나리오 누적으로 자연스럽게 문이 많아져 `**/*.test.{ts,tsx}`에서 끈다.

**Configuration**

- `max` (int, default: `10`): 함수당 최대 문 수
...
**베스트 프랙티스.** 변경 가능한 export는 모듈 경계의 명시적 인터페이스를 모호하게 하고 consumer가 값의 변경 시점을 추적하기 어려우므로, 상태를 외부에 공유해야 하면 getter 함수나 명시적인 스토어 API로 제공한다.
...
`functions/**/*.ts`와 `scripts/**/*.ts`는 재정의에서 규칙이 꺼져 있어 Node.js 내장 모듈 사용이 허용된다.
...
`strict: true`로 `await` 이후 `.then()` 체이닝도 검출된다.
...
**베스트 프랙티스.** `interface`는 선언 병합이 가능해 라이브러리 확장과 호환되고 IDE의 호버와 오류 표시도 더 짧으며, 유니언이나 튜플처럼 객체로 표현할 수 없는 형태는 `type`을 그대로 쓸 수 있어 규칙 충돌 없이 양립한다.
...
`catch` 절의 변수 이름과 `Promise#catch`, `Promise#then`의 rejection 핸들러 매개변수 이름을 표준 이름(기본 `error`)으로 통일한다.

**취향.** 자동 수정으로 정리되는 가벼운 일관성 규칙이고, `_`(미사용)나 `_error`(사용하되 이름을 숨김) 패턴은 규칙이 자동으로 허용해 best-effort 실행을 표현하는 빈 `catch` 패턴을 방해하지 않는다.

**Configuration**

- `name` (string, default: `"error"`): 표준 오류 변수 이름
...
`new Set([])`, `new Map(foo ?? [])`처럼 컬렉션 생성자에 의미 없는 빈 iterable이나 nullish 폴백을 전달하는 패턴을 금지한다. 컬렉션 생성자는 인자가 없거나 nullish일 때 빈 컬렉션을 생성하므로 폴백이 불필요하다.

**베스트 프랙티스.** 자동 수정으로 정리되는 가벼운 규칙이고 패턴이 거의 항상 잘못된 의도(필요 없는 방어 코드)에서 비롯되어 오탐 부담이 적다.
...
`"UTF-8"`, `"utf-8"`, `"utf8"`처럼 텍스트 인코딩 식별자 표기가 섞이면 `grep`과 정렬에서 같은 인코딩이 다른 결과로 처리된다. 기본 옵션으로 대시 없는 소문자 형태(`"utf8"`, `"ascii"`)로 통일한다.

**취향.** 자동 수정으로 정리되는 표기 일관성 규칙이라 오탐이 사실상 없고, 파일 I/O나 `Buffer.toString` 등에서 인코딩 인자가 한 가지 모양으로 등장해 가독성이 올라간다.
